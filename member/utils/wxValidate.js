...
class wxValidate {
    ...
    defaultMessage(param, rule) {
        let message = rule.message || this.defaults.messages[rule.method]
     ...
     */
    checkParam(type, param, rules, event) {
        let data
        if (type == "form") { //表单模式
            // 缓存表单数据对象
            this.scope = event
        ...
        for (let method in rules) {
            ...
            if (this.isValidMethod(method)) {
                ...
                const rule = {
                    method: method,
                    parameters: rules[method].value,
                    message: rules[method].message
                }
    ...
    checkForm(event) {
        ...
        for (let param in this.rules) {
            this.setView(param)
            this.checkParam("form", param, this.rules[param], event)
        }
        return this.valid() ? null : this.errorList[0]
    }
    ...
    checkData(data) {
        ...
        for (let param in this.rules) {
            this.setView(param)
            this.checkParam("data", param, this.rules[param], data)
        }
        return this.valid() ? null : this.errorList[0]
    }
